                    ...
                    if (now - boss.laserHitCooldown >= boss.laserHitDelay) {
                        for (let p of boss.collisionPoints) {
                            ...
                            if (dist(px, py, w.x, w.y) < w.radius) {
                                ...
                                console.log(`Lightning hit ${p.vulnerable ? "VULNERABLE" : "non-vulnerable"} point. Boss HP: ${boss.bossHealth}`);
    ...
    if (gameState === "bossState" && boss) {
        for (let p of boss.collisionPoints) {
            ...
            if (dist(px, py, w.x, w.y) < 20) {
                boss.bossHealth -= p.vulnerable ? 25 : 8;
                console.log(`Bullet/Missile hit ${p.vulnerable ? "VULNERABLE" : "non-vulnerable"} point. Boss HP: ${boss.bossHealth}`);
                weapons.active.splice(index, 1);
                return;
            }
        }
    ...
    if (gameState === "bossState" && boss) {
        for (let p of boss.collisionPoints) {
            ...
            if (dist(px, py, w.x, w.y) < R) {
                boss.bossHealth -= p.vulnerable ? 25 : 8;
                console.log(`Mine hit ${p.vulnerable ? "VULNERABLE" : "non-vulnerable"} point. Boss HP: ${boss.bossHealth}`);
            }
        }
    }
}
    ...
    if (gameState === "bossState" && boss) {
        ...
        if (now - boss.laserHitCooldown >= boss.laserHitDelay) {
            for (let p of boss.collisionPoints) {
                ...
                const perp = Math.abs(dx * sin - dy * cos);
                if (dot > 0 && perp < 24) {
                    boss.bossHealth -= p.vulnerable ? 25 : 8; // ← was always 5, skipped non-vulnerable
                    console.log(`Laser hit ${p.vulnerable ? "VULNERABLE" : "non-vulnerable"} point. Boss HP: ${boss.bossHealth}`);
                    boss.laserHitCooldown = 500;
                }
            }
        }
    }
